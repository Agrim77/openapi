...
const regex2 = /Question \d+ : (.*?)(?=Question \d+ :|$)(?:.*\n)*?Answer \d+ : (.*?)(?=\n\n|$)/gs;
  const matches = inputText.matchAll(regex2);
  
  const result = [];
  
  for (const match of matches) {
    const ques = match[1].trim();
    const ans = match[2].trim();
    
    result.push({ ques, ans });
  }
  


//   const regex2 = /Question (\d+) : (.+?)\n\s+Answer \d+ : (.+?)(?=\n\s+Question|$)/gs;
// const result = [];
// let match;

// while ((match = regex2.exec(c_inputText))) {
